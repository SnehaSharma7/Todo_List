...
const hasuraPathKey =
  `${process.env.HASURA_INTEGRATION_HOST}/v1/graphql`.replace(
    /(\r\n|\n|\r)/gm,
    ""
  );
const hasuraObject = {
  ...
  schema: [
    {
      [hasuraPathKey]: {
        headers: {
          "x-hasura-admin-secret": `${process.env.HASURA_ADMIN_SECRET}`.replace(
            /(\r\n|\n|\r)/gm,
            ""
          ),
        },
...
// {
//   overwrite: true,
//   schema: `${process.env.GRAPH_MIDDLEWARE_HOST.replace(/(\r\n|\n|\r)/gm, "")}/${process.env.GRAPH_MIDDLEWARE_API_KEY.replace(/(\r\n|\n|\r)/gm, "")}/${process.env.GRAPH_MIDDLEWARE_PATH.replace(/(\r\n|\n|\r)/gm, "")}`,
